...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      const url = `http://localhost:8181/assJob/findByStuId/1/6/` + account.username
    ...
    add () {
      const _this = this
      let account = JSON.parse(localStorage.getItem('user'))
      axios.get(`http://localhost:8181/assJob/findByAssistant/` + account.username).then(function (resp) {
        _this.ruleForm = resp.data
        _this.ruleForm.weekTimes = ''
        _this.ruleForm.time = ''
        _this.ruleForm.id = ''
        _this.ruleForm.week = ''
        _this.ruleForm.jobContent = ''
        console.log('wo' + resp.data)
      })
      this.list_addDialog = true
      _this.getData()
    },
    // 添加
    submitForm (formName) {
